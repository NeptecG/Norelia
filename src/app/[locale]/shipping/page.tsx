...
import { ShippingReturnsTabs } from '@/components/info/shipping-returns-tabs'
...
export default async function ShippingReturnsPage() {
  ...
  const shippingPanel = (
    <>
      <p className="font-body text-[15px] text-on-surface-muted leading-[1.85] tracking-[0.02em] mb-16">{t('intro')}</p>
      ...
      {METHODS.map(({ titleKey, timeKey, priceKey, detailKey }, i) => (
        <div key={titleKey} className="border-t border-border-subtle pt-10 mb-12">
          ...
          <div className="flex justify-between items-start gap-4 mb-5 flex-wrap">
            <h2 className="font-display text-2xl tracking-[0.06em] text-on-surface">{t(titleKey)}</h2>
  ...
  const returnsPanel = (
    <>
      <p className="font-body text-[15px] text-on-surface-muted leading-[1.85] tracking-[0.02em] mb-16">{r('intro')}</p>

      {RETURN_SECTIONS.map(({ titleKey, detailKey }, i) => (
        <div key={titleKey} className="border-t border-border-subtle pt-10 mb-12">
          <p className="font-display text-[11px] tracking-[0.3em] text-on-surface/25 mb-2.5">0{i + 1}</p>
          <h2 className="font-display text-2xl tracking-[0.06em] text-on-surface mb-5">{r(titleKey)}</h2>
          <p className="font-body text-[13px] text-on-surface-muted leading-[1.85] tracking-[0.02em] border-l-2 border-border-subtle pl-5">
            {r(detailKey)}
          </p>
        </div>
    ...
    <main className="min-h-screen bg-surface">
      ...
      <section className="max-w-[1440px] mx-auto px-4 md:px-[60px] py-20">
        <div className="max-w-[760px]">
          <ShippingReturnsTabs
            shippingLabel={t('tabShipping')}
            returnsLabel={t('tabReturns')}
